...
date: "June 14, 2026"
commit: "e643a8d4c2c802490976b538dd009b351b1c8dda"
...
- **Manual review** — line-by-line reading of `PoolFactory.sol` and `TSwapPool.sol`, focused on the
  AMM accounting, fee math, slippage protection, and the core `x * y = k` invariant.
- **Dynamic testing** — Foundry unit tests and proof-of-concept exploits, plus fuzz/invariant testing
  to validate that swaps preserve the constant-product invariant.
- **Weird-ERC20 analysis** — reasoning about non-standard ERC20 behaviors and their effect on the
  pool accounting.
...
| H-2   | No slippage protection in `swapExactOutput`       | High     | Open   |
| H-3   | `sellPoolTokens` swaps the wrong direction        | High     | Open   |
| H-4   | Swap bonus breaks the `x * y = k` invariant       | High     | Open   |
| M-1   | `deposit` ignores its `deadline` parameter        | Medium   | Open   |
| L-1   | `LiquidityAdded` event arguments out of order     | Low      | Open   |
| L-2   | `swapExactInput` always returns zero               | Low      | Open   |
| I-1   | Unused error in `PoolFactory`                      | Info     | Open   |
| I-2   | Missing zero-address check in `PoolFactory`        | Info     | Open   |
| I-3   | Use `.symbol()` instead of `.name()`               | Info     | Open   |
| I-4   | `Swap` event is under-indexed                       | Info     | Open   |
| I-5   | Missing zero-address check in `TSwapPool`           | Info     | Open   |
